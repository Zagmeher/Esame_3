...
function nameImg_P(){
    $img_po = null;
    $conn = mysqli_connect('31.11.39.226', 'Sql1872203', 'Zagmeher_92!', 'Sql1872203_1');
    if ($conn) {
...
function adduser() {
    ...
    <div id="adduser-form" style="
        ...
        max-height: 90vh;
    ">
        <form class="adduser" action="backend.php" method="post" style="display: flex; flex-direction: column; gap: 12px;">
            ...
            <input type="password" name="addPsw" id="addPsw" required style="width: 60%;">
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <form method="post" action="backend.php">
                    <tr id="row-'.$id.'">
                        ...
                        <td style="padding:8px; border:1px solid #ccc;">
                            <input type="text" name="updPsw" value="'.$password.'" style="width:90%;" disabled>
                        </td>
                        <td style="padding:8px; border:1px solid #ccc;">
...
function addport() {
    ...
    if (isset($_POST['submit_addport'])) {
        ...
        if (
            ...
            $ftp_server = 'ftp.angeloiandolo.it';
            $ftp_user = '13563276@aruba.it';
            $ftp_pass = 'Zagmeher_92';
            $ftp_remote_dir = 'www.angeloiandolo.it/img/';
...
function updport() {
    if (isset($_POST['submit_updport'])) {
        ...
        $conn = mysqli_connect('31.11.39.226', 'Sql1872203', 'Zagmeher_92!', 'Sql1872203_1');
        if ($conn) {
            $stmt = mysqli_prepare($conn, "UPDATE portfolio SET titolo_p=?, descr_p=? WHERE ID_po=?");
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssi", $titolo_port, $descr_port, $id);
                $executed = mysqli_stmt_execute($stmt);
                if ($executed) {
                    echo '
                    <div id="success-message" style="
                        ...
                        text-align: center;
                    ">
                        Elemento portfolio aggiornata con successo!
                    </div>
                    <script>
                        setTimeout(function() {
                            var msg = document.getElementById("success-message");
